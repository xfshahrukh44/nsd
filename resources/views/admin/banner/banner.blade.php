              ...
              @if(count($banners) > 0)
                @foreach($banners as $banner)
                  <tr role="row" class="odd">
                    ...
                    <!-- subtitle -->
                    <td class="{{'subtitle'.$banner->id}}" width="500">{{$banner->subtitle}}</td>
                    ...
                    <!-- image (fancybox) -->
                    <td class="{{'image'.$banner->id}}" width="140">
                        <a class="fancybox" href="{{($banner->image) ? (asset('img/banners') . '/' . $banner->image) : (asset('img/noimg.jpg'))}}">
                            <img src="{{($banner->image) ? (asset('img/banners') . '/' . $banner->image) : (asset('img/noimg.jpg'))}}" width="60%" alt="">
                        </a>
                    </td>
...
$(document).ready(function(){
    ...
    $('.editButton').on('click', function(){
        ...
        if(banner.image){
          var src = `<?php echo(asset('img/banners') . '/temp'); ?>`;
          src = src.replace('temp', banner.image);
        }
        else{
