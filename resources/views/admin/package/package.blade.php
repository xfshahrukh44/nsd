...
<div class="row mb-2">
  <div class="col-sm-6">
    <h1 class="m-0 text-dark"><i class="nav-icon fas fa-money-bill-wave"></i> Packages</h1>
  </div>
</div>
    ...
    <div class="card">
      <div class="card-header">
        <div class="card-tools">
            <!-- <button class="btn btn-success" id="add_item" data-toggle="modal" data-target="#addPackageModal">
                <i class="fas fa-plus"></i>
            </button> -->
        </div>
        <!-- search bar -->
        <form action="{{route('search_packages')}}" class="form-wrapper">
          <div class="row">
              <!-- search bar -->
              ...
              @if(count($packages) > 0)
                @foreach($packages as $package)
                  <tr role="row" class="odd">
                    ...
                    <!-- title -->
                    <td class="{{'title'.$package->id}}">{{$package->title}}</td>

                    <!-- amount -->
                    <td class="{{'amount'.$package->id}}">{{'$' . $package->amount}}</td>

                    <!-- actions -->
                    <td width="100">
                        <!-- Detail -->
                        <!-- <a href="#" class="detailButton" data-id="{{$package->id}}">
                          <i class="fas fa-eye green ml-1"></i>
                        </a> -->
                        <!-- Edit -->
                        <a href="#" class="editButton" data-id="{{$package->id}}">
                ...
                @endforeach
              @else
                <tr><td colspan="3"><h6 align="center">No package(s) found</h6></td></tr>
              @endif
            </tbody>
            <tfoot>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      ...
      <form method="POST" action="{{route('package.store')}}" enctype="multipart/form-data">
        @csrf
        @include('admin.package.package_master')
...
<div class="modal fade" id="editPackageModal" tabindex="-1" role="dialog" aria-labelledby="editPackageModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      ...
      <form id="editForm" method="POST" action="{{route('package.update', 1)}}" enctype="multipart/form-data">
        <!-- hidden input -->
        @method('PUT')
...
<!-- Detail view -->
<div class="modal fade" id="viewPackageModal" tabindex="-1" role="dialog" aria-labelledby="addPackageModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <ul class="nav nav-pills nav-fill" id="myTab" role="tablist">
              <li class="nav-item" role="presentation">
                <a class="nav-link active bci" data-toggle="tab" href="#bci">Basic Package Information</a>
              </li>
              <li class="nav-item" role="presentation" >
...
<div class="modal fade" id="deletePackageModal" tabindex="-1" role="dialog" aria-labelledby="deletePackageModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <form id="deleteForm" method="POST" action="{{route('package.destroy', 1)}}">
        <!-- hidden input -->
        @method('DELETE')
        @csrf
...
$(document).ready(function(){
    ...
    function fetch_package(id){
        $.ajax({
            url: '<?php echo(route("package.show", 0)); ?>',
            type: 'GET',
            data: {id: id},
            dataType: 'JSON',
            async: false,
    ...
    $('.editButton').on('click', function(){
        ...
        // title
        $('#editForm .title').val(package.title ? package.title : '');

        // amount
        $('#editForm .amount').val(package.amount ? package.amount : '');
    ...
    $('.detailButton').on('click', function(){
      ...
      var id = $(this).data('id');
      fetch_package(id);
      // var package = $(this).data('object');
      $('.name').html(package.name ? package.name : '');
      $('.contact_number').html(package.contact_number ? package.contact_number : '');
      $('.whatsapp_number').html(package.whatsapp_number);
      if(package.shop_keeper_picture){
      ...
      $('.type').html(package.type ? package.type : '');
      $('.shop_name').html(package.shop_name ? package.shop_name : '');
      $('.shop_number').html(package.shop_number ? package.shop_number : '');
      $('.floor').html(package.floor ? package.floor : '');
      $('.area').html((package.market && package.market.area) ? package.market.area.name : '');
      $('.market').html(package.market ? package.market.name : '');
      if(package.shop_picture){
      ...
      if(package.package_images.length > 0){
          for(var i = 0; i < package.package_images.length; i++){
          $('.gallery_wrapper').append(`<div class="col-md-4 mb-3"><a target="_blank" href="{{asset('img/package_images')}}/`+package.package_images[i].location+`" class="col-md-12"><img class="col-md-12 shop_keeper_picture" src="{{asset('img/package_images')}}/`+package.package_images[i].location+`"></a><button class="btn btn_del_package_image" value="`+package.package_images[i].id+`" type="button"><i class="fas fa-trash red ml-1"></i></button></div>`);
          }
      }
      $('.status').html(package.status ? package.status : '');
      $('.visiting_days').html(package.visiting_days ? package.visiting_days : '');
      $('.cash_on_delivery').html(package.cash_on_delivery ? package.cash_on_delivery : '');
      $('.opening_balance').html(package.opening_balance ? ("Rs. " + package.opening_balance.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")) : '');
      $('.business_to_date').html(package.business_to_date ? ("Rs. " + package.business_to_date.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")) : '');
      $('.outstanding_balance').html(package.outstanding_balance ? ("Rs. " + package.outstanding_balance.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")) : '');
      // $('.special_discount').html(package.special_discount ? ("Rs. " + package.special_discount) : '');
      $('#viewPackageModal').modal('show');
      });
      // delete
      $('.deleteButton').on('click', function(){
      var id = $(this).data('id');
      $('#deleteForm').attr('action', "{{route('package.destroy', 1)}}");
      $('#deleteForm .hidden').val(id);
      $('#deletePackageModalLabel').text('Delete Package: ' + $('.name' + id).html() + "?");
