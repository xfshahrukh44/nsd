...
<div class="row mb-2">
  <div class="col-sm-6">
    <h1 class="m-0 text-dark"><i class="nav-icon fas fa-text-width"></i> Articles</h1>
  </div>
</div>
  ...
  <div class="col-md-12 col-sm-12">
    <div class="card">
      <div class="card-header">
        ...
        <!-- search bar -->
        <form action="{{route('search_articles')}}" class="form-wrapper">
          <div class="row">
              <!-- search bar -->
              ...
              @if(count($articles) > 0)
                @foreach($articles as $article)
                  <tr role="row" class="odd">
                    <!-- image (fancybox) -->
                    <td class="{{'image'.$article->id}}" width="140">
                        <a class="fancybox" href="{{($article->image) ? (asset('img/articles') . '/' . $article->image) : (asset('img/noimg.jpg'))}}">
                            <img src="{{($article->image) ? (asset('img/articles') . '/' . $article->image) : (asset('img/noimg.jpg'))}}" width="60%" alt="">
                        </a>
                    </td>

                    <!-- content (with see more collapse) -->
                    <td class="{{'content'.$article->id}}" width="500">
                    ...
                    <td width="100">
                        <!-- Detail -->
                        <!-- <a href="#" class="detailButton" data-id="{{$article->id}}">
                          <i class="fas fa-eye green ml-1"></i>
                        </a> -->
                        <!-- Edit -->
                        <a href="#" class="editButton" data-id="{{$article->id}}">
                          <i class="fas fa-edit blue ml-1"></i>
                        </a>
                        <!-- Delete -->
                        <a href="#" class="deleteButton" data-id="{{$article->id}}">
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      ...
      <form method="POST" action="{{route('article.store')}}" enctype="multipart/form-data">
        @csrf
        @include('admin.article.article_master')
...
$(document).ready(function(){
    ...
    function fetch_article(id){
        $.ajax({
            url: '<?php echo(route("article.show", 0)); ?>',
            type: 'GET',
            data: {id: id},
            dataType: 'JSON',
            async: false,
    ...
    $('.editButton').on('click', function(){
        ...
        if(article.image){
          var src = `<?php echo(asset('img/articles') . '/temp'); ?>`;
          src = src.replace('temp', article.image);
        }
        else{
        ...
        // content
        $('#editForm .content').val(article.content ? article.content : '');

        // link
        $('#editForm .link').val(article.link ? article.link : '');
    ...
    $('.detailButton').on('click', function(){
      ...
      var id = $(this).data('id');
      $('#deleteForm').attr('action', "{{route('article.destroy', 1)}}");
