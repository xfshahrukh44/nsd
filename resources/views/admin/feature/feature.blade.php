...
<div class="row mb-2">
  <div class="col-sm-6">
    <h1 class="m-0 text-dark"><i class="nav-icon fas fa-text-width"></i> Features</h1>
  </div>
</div>
    ...
    <div class="card">
      <div class="card-header">
        <div class="card-tools">
            <button class="btn btn-success" id="add_item" data-toggle="modal" data-target="#addFeatureModal">
                <i class="fas fa-plus"></i>
            </button>
        </div>
        <!-- search bar -->
        <form action="{{route('search_features')}}" class="form-wrapper">
          <div class="row">
              <!-- search bar -->
              ...
              @if(count($features) > 0)
                @foreach($features as $feature)
                  <tr role="row" class="odd">
                    ...
                    <td class="{{'content'.$feature->id}}" width="500">
                    ...
                    <td width="100">
                        <!-- Detail -->
                        <!-- <a href="#" class="detailButton" data-id="{{$feature->id}}">
                          <i class="fas fa-eye green ml-1"></i>
                        </a> -->
                        <!-- Edit -->
                        <a href="#" class="editButton" data-id="{{$feature->id}}">
                          <i class="fas fa-edit blue ml-1"></i>
                        </a>
                        <!-- Delete -->
                        <a href="#" class="deleteButton" data-id="{{$feature->id}}">
                ...
                @endforeach
              @else
                <tr><td colspan="4"><h6 align="center">No feature(s) found</h6></td></tr>
              @endif
            </tbody>
            <tfoot>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      ...
      <form method="POST" action="{{route('feature.store')}}" enctype="multipart/form-data">
        @csrf
        @include('admin.feature.feature_master')
...
<div class="modal fade" id="editFeatureModal" tabindex="-1" role="dialog" aria-labelledby="editFeatureModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      ...
      <form id="editForm" method="POST" action="{{route('feature.update', 1)}}" enctype="multipart/form-data">
        <!-- hidden input -->
        @method('PUT')
...
<!-- Detail view -->
<div class="modal fade" id="viewFeatureModal" tabindex="-1" role="dialog" aria-labelledby="addFeatureModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Feature Detail</h5>
            ...
            <ul class="nav nav-pills nav-fill" id="myTab" role="tablist">
              <li class="nav-item" role="presentation">
                <a class="nav-link active bci" data-toggle="tab" href="#bci">Basic Feature Information</a>
              </li>
              <li class="nav-item" role="presentation" >
...
<div class="modal fade" id="deleteFeatureModal" tabindex="-1" role="dialog" aria-labelledby="deleteFeatureModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <form id="deleteForm" method="POST" action="{{route('feature.destroy', 1)}}">
        <!-- hidden input -->
        @method('DELETE')
        @csrf
...
$(document).ready(function(){
    ...
    var feature = "";
    // fetch feature
    function fetch_feature(id){
        $.ajax({
            url: '<?php echo(route("feature.show", 0)); ?>',
            type: 'GET',
            data: {id: id},
            dataType: 'JSON',
            async: false,
    ...
    $('.editButton').on('click', function(){
        ...
        // content
        $('#editForm .content').val(feature.content ? feature.content : '');
    ...
    $('.detailButton').on('click', function(){
      ...
      var id = $(this).data('id');
      fetch_feature(id);
      // var feature = $(this).data('object');
      $('.name').html(feature.name ? feature.name : '');
      $('.contact_number').html(feature.contact_number ? feature.contact_number : '');
      $('.whatsapp_number').html(feature.whatsapp_number);
      if(feature.shop_keeper_picture){
      ...
      $('.type').html(feature.type ? feature.type : '');
      $('.shop_name').html(feature.shop_name ? feature.shop_name : '');
      $('.shop_number').html(feature.shop_number ? feature.shop_number : '');
      $('.floor').html(feature.floor ? feature.floor : '');
      $('.area').html((feature.market && feature.market.area) ? feature.market.area.name : '');
      $('.market').html(feature.market ? feature.market.name : '');
      if(feature.shop_picture){
      ...
      if(feature.feature_images.length > 0){
          for(var i = 0; i < feature.feature_images.length; i++){
          $('.gallery_wrapper').append(`<div class="col-md-4 mb-3"><a target="_blank" href="{{asset('img/feature_images')}}/`+feature.feature_images[i].location+`" class="col-md-12"><img class="col-md-12 shop_keeper_picture" src="{{asset('img/feature_images')}}/`+feature.feature_images[i].location+`"></a><button class="btn btn_del_feature_image" value="`+feature.feature_images[i].id+`" type="button"><i class="fas fa-trash red ml-1"></i></button></div>`);
          }
      }
      $('.status').html(feature.status ? feature.status : '');
      $('.visiting_days').html(feature.visiting_days ? feature.visiting_days : '');
      $('.cash_on_delivery').html(feature.cash_on_delivery ? feature.cash_on_delivery : '');
      $('.opening_balance').html(feature.opening_balance ? ("Rs. " + feature.opening_balance.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")) : '');
      $('.business_to_date').html(feature.business_to_date ? ("Rs. " + feature.business_to_date.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")) : '');
      $('.outstanding_balance').html(feature.outstanding_balance ? ("Rs. " + feature.outstanding_balance.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")) : '');
      // $('.special_discount').html(feature.special_discount ? ("Rs. " + feature.special_discount) : '');
      $('#viewFeatureModal').modal('show');
      });
      // delete
      $('.deleteButton').on('click', function(){
      var id = $(this).data('id');
      $('#deleteForm').attr('action', "{{route('feature.destroy', 1)}}");
      $('#deleteForm .hidden').val(id);
      $('#deleteFeatureModalLabel').text('Delete Feature: ' + $('.name' + id).html() + "?");
